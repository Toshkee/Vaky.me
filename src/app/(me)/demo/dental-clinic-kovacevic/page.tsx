...
/* The table of contents. The numerals are the page's spine: they head every
   section, and the areas index reuses the same figures one level down — which
   is why the register in `#tim` can cite an area by number and be understood. */
const contents = [
  { href: "#oblasti", numeral: "01", label: "Oblasti" },
  { href: "#prvi-korak", numeral: "02", label: "Prvi korak" },
  { href: "#tim", numeral: "03", label: "Tim" },
  { href: "#lokacije", numeral: "04", label: "Lokacije" },
];

/**
 * The report's ruled margin. Every section hangs off it: a short label or
 * numeral in the narrow left column, a hairline down its edge, the content in
 * the right. It replaces the card — nothing on this page needs a box, because
 * everything already sits on a shared vertical rule.
 *
 * The rail numerals are `aria-hidden`: they repeat what the heading beside them
 * and the contents bar above already say. The area numerals inside `#oblasti`
 * and the citations in `#tim` are read out, because those carry meaning of
 * their own.
 */
function Ruled({
  rail,
  ...
  return (
    <div className="mx-auto grid max-w-6xl gap-y-5 px-5 sm:px-8 md:grid-cols-[6rem_minmax(0,1fr)] md:gap-y-0 lg:grid-cols-[7.5rem_minmax(0,1fr)]">
      {/* The margin rule turns: a hairline down the edge of the rail on wide
          ...
          content instead of beside it. It also keeps a section numeral from
          reading as one more entry in the list that follows. */}
      <div
        className={`${serif} ${ink} border-b pb-4 text-xl leading-none ${line} md:border-b-0 md:border-r md:pb-0 md:pr-6 md:text-right lg:pr-10`}
      >
        {rail}
      </div>
  ...
  return (
    ...
    >
      ...
      <header className="border-b border-[var(--dent-line)]">
        ...
        <nav aria-label="Sadržaj strane" className="border-t border-[var(--dent-line)]">
          ...
          >
            {contents.map((entry) => (
              <li key={entry.href}>
                <a href={entry.href} className={`flex min-h-12 items-center gap-2.5 pl-4 ${focus}`}>
                  <span className={`${serif} text-sm text-[var(--dent-teal)]`}>{entry.numeral}</span>
      ...
      <main id="vrh">
        ...
        {/* 01 — the index. Wide report rows, not tiles: numeral, area, two lines,
            and the names the public record attaches to that area. */}
        <section
          id="oblasti"
          className="scroll-mt-6 border-b border-[var(--dent-line)] py-14 sm:py-20"
        >
          <Ruled rail={<span aria-hidden="true">01</span>}>
            <h2
            ...
            <p className="mt-4 max-w-[36rem] leading-relaxed text-[var(--dent-steel)]">
              Tri oblasti u kojima ordinacija radi. Brojevi se ponavljaju u registru tima niže na
              strani, pa se svaka oblast može pratiti do imena.
            </p>
            ...
            <ol className="mt-10 border-b border-[var(--dent-line)]">
              {fields.map((field) => (
                <li
                  key={field.id}
                  className="grid gap-x-8 gap-y-3 border-t border-[var(--dent-line)] py-7 sm:grid-cols-[3rem_minmax(0,1fr)] lg:grid-cols-[3rem_minmax(0,1fr)_13rem]"
                >
                  <p className={`${serif} text-[1.35rem] leading-none text-[var(--dent-teal)]`}>
                    {field.id}
                  </p>
                  <div>
                    ...
                    <p className="mt-2.5 leading-relaxed text-[var(--dent-steel)]">{field.body}</p>
                  </div>
                  {/* Between sm and lg the row is two columns wide, so this has
                      to be pushed under the description rather than auto-placed
                      into the 3rem numeral column beside it. */}
                  <p className="text-sm leading-relaxed text-[var(--dent-steel)] sm:col-start-2 lg:col-start-3 lg:text-right">
        ...
        {/* 02 — the shortest section on the page, on a half-tone band. Three
            marks on one scale: what happens, in the order it happens. */}
        <section
          id="prvi-korak"
          className="scroll-mt-6 border-b border-[var(--dent-line)] bg-[var(--dent-paper)] py-12 sm:py-14"
        >
          <Ruled rail={<span aria-hidden="true">02</span>}>
            <h2
              className={`${serif} text-[clamp(1.7rem,4vw,2.4rem)] leading-[1.1] tracking-[-0.01em]`}
            >
              Prvi korak
            ...
            <ol className="mt-10 grid gap-8 sm:grid-cols-3 sm:gap-7">
              {steps.map((step) => (
                <li
                  key={step.title}
                  className={`${styles.tick} relative border-t border-[var(--dent-line-strong)] pt-5`}
                >
        ...
        {/* 03 — the register. The same three names the public record lists, each
            cited back to an area numeral from 01. */}
        <section id="tim" className="scroll-mt-6 border-b border-[var(--dent-line)] py-14 sm:py-20">
          <Ruled rail={<span aria-hidden="true">03</span>}>
            <h2
              className={`${serif} text-[clamp(1.7rem,4vw,2.4rem)] leading-[1.1] tracking-[-0.01em]`}
            >
              Tim
            ...
            <ul className="mt-10">
              {team.map((doctor) => (
                <li
                  key={doctor.name}
                  className="grid items-baseline gap-x-8 gap-y-1.5 border-t border-[var(--dent-line)] py-6 sm:grid-cols-[18rem_minmax(0,1fr)_auto]"
                >
                  <p className={`${serif} text-[1.3rem] leading-tight tracking-[-0.01em]`}>
                    {doctor.name}
                  </p>
                  <p className="text-[var(--dent-steel)]">{doctor.field}</p>
                  <p className="sm:text-right">
                    <span className={`${caps} text-[var(--dent-steel)]`}>Oblast</span>{" "}
                    <span className={`${serif} text-[1.05rem] text-[var(--dent-teal)]`}>
                      {doctor.covers.join(" · ")}
                    </span>
                  </p>
                </li>
            ...
            <div className="border-t border-[var(--dent-line)] pt-5">
              <p className="max-w-[36rem] text-sm leading-relaxed text-[var(--dent-steel)]">
                Oblast 03 vodi se kao rad ordinacije — pojedinačne uloge u njoj nijesu javno
                navedene.
              </p>
            </div>
          </Ruled>
        ...
        {/* 04 — two place names, set large, divided by the same hairline that
        ...
        <section
          ...
          className="scroll-mt-6 border-b border-[var(--dent-line)] py-14 sm:py-20"
        >
          <Ruled rail={<span aria-hidden="true">04</span>}>
            <h2
              className={`${serif} text-[clamp(1.7rem,4vw,2.4rem)] leading-[1.1] tracking-[-0.01em]`}
            >
              Dvije ordinacije
            ...
            <h3 className={`mt-14 ${caps} text-[var(--dent-steel)]`}>Prostor</h3>
            <ul className="mt-5 grid gap-7 sm:grid-cols-2 sm:gap-x-6 sm:gap-y-0">
              {photos.map((photo) => (
                <li key={photo.src}>
                  <figure>
                    <DemoPhoto
                      ...
                      width={photo.width}
                      height={photo.height}
                      sizes="(min-width: 1152px) 452px, (min-width: 640px) 44vw, 88vw"
                      className="w-full border border-[var(--dent-line)] object-cover"
                    />
                    <figcaption
