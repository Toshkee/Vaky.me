...
/* Playwright's iPhone profiles are Chromium with a spoofed UA, which is the
   wrong engine for exactly the things phones break on â€” :active under touch,
...
for (const [engineName, engine] of engines) {
  ...
  for (const dev of ["iPhone SE", "iPhone 13"]) {
    ...
    console.log(`\n=== ${engineName} Â· ${dev} (${devices[dev].viewport.width}px) ===`);
    ...
    for (const t of targets) {
      ...
      const d = await page.evaluate(() => {
        ...
        for (const el of document.querySelectorAll("body *")) {
          ...
          if ((r.right > vw + 1 || r.left < -1) && !clipped(el)) {
            out.push(
              `${el.tagName.toLowerCase()}.${(el.getAttribute("class") || "").split(" ")[0]} [${Math.round(r.left)}â†’${Math.round(r.right)}] "${(el.textContent || "").trim().slice(0, 28)}"`,
            );
          }
        }
      ...
      d.out.forEach((o) => console.log(`      â†³ ${o}`));
